...
export function FilterSelect(props: FilterSelectProps) {
  ...
  const activeSubs = options.find(o => o.slug === activeSlug)?.subcategories || [];
  ...
  if (variant === 'radio') {
    ...
    if (v.length > 0) {
      label = `${v.length} ${v.length > 1 ? pluralize(placeholder) : placeholder}`;
    }
  }
  ...
  const toggleCheck = (tag: Tag) => {
    setLocalTags(prev => {
      ...
      if (isSelected) {
        ...
        // Check parent + all children not already selected
